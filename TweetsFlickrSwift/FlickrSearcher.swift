...
//  TweetsFlickrSwift
//
//  Created by optimusmac4 on 8/8/15.
...
let apiKey = "06db29b622a83e1eddc6e20d84dbff2c"
...
class FlickrPhoto : Equatable {             //Modal class which will have all the detail of the  pictures fetched
...
class Flickr {
    ...
    func searchFlickrForTerm(searchTerm: String, completion : (results: FlickrSearchResults?, error : NSError?) -> Void){     //This will fetch images from flickr
        let searchURL = flickrSearchURLForSearchTerm(searchTerm)    //Calling for creating the search URL
    ...
    private func flickrSearchURLForSearchTerm(searchTerm:String) -> NSURL {
        ...
        let URLString = "https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=\(apiKey)&text=\(escapedTerm)&per_page=20&format=json&nojsoncallback=1"      //This is the search link which will search photos according to the search key
